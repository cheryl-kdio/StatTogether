...
qaly<-qaly %>%
  mutate(ARPREMARAISON = case_when(
    ...
    ARPREMARAISON == 11 ~ "réévaluation moléculaire",
    TRUE ~ as.character(ARPREMARAISON) # Garde la valeur originale si non correspondante
  ))

#reorder factor
qaly$ARPREMARAISON <- factor(qaly$ARPREMARAISON, levels = c("rechute", "deces", "EIG",  "souhait malade", "retrait cst", "réévaluation moléculaire", "autre"))
...
qaly %>% 
  ...
  mutate(Variable = factor(Variable,levels = c("randodt", "prephasdt", "C1dt", "Datemax"),labels = c("Date d'inclusion", "Date de pré-phase","Date de chimiothérapie(Cycle 1)", "Date de fin d'étude"))) %>%
  gt() %>%
  tab_header(
...
```{r partition1}
ct  <- survfit(Surv(delfinct,finct)~1,data=qaly,subset = (R1 == "Intensive arm (A)"))
ct2  <- survfit(Surv(delfinct2,finct2)~1,data=qaly,subset = (R1 == "Intensive arm (A)"))
sv  <- survfit(Surv(suivi,as.numeric(as.character(dc)))~1,data=qaly,subset = (R1 == "Intensive arm (A)"))
efs <- survfit(Surv(delpfs,as.numeric(as.character(pfs)))~1, data=qaly,subset = (R1 == "Intensive arm (A)"))
```


```{r fig.width=6, fig.height=5.5}
fit<-list(CT = ct, CT2 = ct2, SV=sv, EFS=efs)
library(survminer)
...
df %>% mutate(strata = sub("x=", "", strata)) %>%
  ...
  ggplot(aes(time, CT)) +
  geom_step(aes(group = "CT")) +
  geom_ribbon(aes(ymin = 0, ymax = CT, fill = "1"),alpha=0.5)+
  geom_step(aes(y = CT2), group = "CT2",linewidth=0.9, colour="#0077b6") +
  geom_ribbon(aes(ymin = CT, ymax = CT2, fill = "2"),alpha=0.5) +
  geom_step(aes(y = EFS),group = "EFS",linewidth=0.9, colour="#48cae4") +
  geom_ribbon(aes(ymin = CT2, ymax = EFS, fill = "3"),alpha=0.5)+
  geom_step(aes(y = SV), group = "SV",linewidth=0.9, colour="#caf0f8")+
  geom_ribbon(aes(ymin = EFS, ymax = SV, fill = "4"),alpha=0.5)+
  scale_fill_manual(values = c("1" = "#03045e", "2" = "#0077b6", "3" = "#48cae4", "4" = "#caf0f8")) +
  labs(x = "Time", y = "Survival Probability", group = "Duration", fill = "Events") +
  theme_minimal() +
