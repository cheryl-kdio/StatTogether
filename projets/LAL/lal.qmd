...
Au moment de l'analyse, nous observons que la moitié des participants ont prolongé leur participation à l'étude pour une durée supérieure à 46 mois.
...
INTRODUIRE CENSURE ??? Notons que ces interruptions sont la cause de censure outre le décès des individus en analyse de survie. En effet, le décès étant l'évènement d'intérêt, lorsque les individus partent de l'étude sans avoir atteint l'évènement, ils sont censurés. De la même manière, lorsque l'étude se termine sans que l'évènement ne se soit produit, les individus sont censurés.
...
L’objectif est de d'évaluer la possibilité de réduire l'intensité du traitement habituel des patients, traditionnellement associée à une forte toxicité, au profit d'un traitement plus ciblé et moins toxique, sans pour autant augmenter le risque de rechute. Pour mesurer cela, nous allons utiliser le principe du TWiST et la méthode Q-TWiST.
...
Une analyse de survie dépend du partitionnement et permet d’analyser les avantages/ désavantages d’un traitement par rapport à un autre en fonction des évènements choisis. Ainsi, nous pourrons faire évoluer ces différents phases en répartition nant pour affiner l’analyse. Nous pourrions, par exemple, inclure une autre phase dans la période sous traitement, i.e une phase dans laquelle le traitement ne présente plus de toxicité.
...
### Données cliniques & biologiques des patients

#### Oncogénétique
Une donnée oncogénétique fait référence à toute information génétique donc biologique liée au développement, à la progression ou à la prédisposition aux cancers.
...
Les données de pré-phase font référence à des informations recueillies avant l'apparition d'une maladie ou d'un état, comme les données génétiques ou les facteurs de risque qui pourraient prédire le développement d'une condition avant qu'elle ne devienne apparente.
