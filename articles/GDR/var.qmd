...
Ici, $P_{i,t}$ est connu tandis que $P_{i,t+h}$ est stochastique. La première idée est de choisir les facteurs comme étant les prix futurs.Dans cette approche, les rendements des actifs sont les facteurs de risque du marché et chaque actif possède son propre facteur de risque.
...
P_{i,t+h} = P_{i,t} (1 + R_{i,t+h})
$$ où $R_{i,t+h}$ est le retour de l'actif entre $t$ et $t+h$.

Nous déduisons que le PnL aléatoire est : \begin{align*}
PnL &= P(t+h,\theta) - P(t,\theta) \\
...
\end{align*} où $W_i = \theta_i P_i(t)$ est le montant investi (ou l'exposition nominale)dans l'actif $i$.
...
\begin{align*}
\sigma^2(\Pi) &= W_t^T \Sigma W_t = 1093.3^2 \left(\frac{1.3611}{100}\right)^2 + 842.8^2 \left(\frac{0.9468}{100}\right)^2 \\
...
Pour la matrice de corrélation en pb (points de base), nous obtenons :
...
$$
Nous obtenons une valeur à risque de 4978\$ pour une période de détention d'un jour.

```{r}
# Définition des écarts-types en points de base convertis en pourcentage
std_devs <- c(0.746, 2.170, 3.264, 3.901, 4.155)/100
...
# Initialisation de la matrice de stockage pour les résultats
cov_matrix <- matrix(0, nrow = 5, ncol = 5)

# Boucles pour calculer et stocker les valeurs dans la matrice
for (i in 1:5) {
  for (j in 1:5) {
    # Calcul de la covariance en utilisant les écarts-types et la matrice de corrélation
...
W_tm <- matrix(c(-4.978, -9.826, -14.437, -18.783, -480.356),nrow = 5)

VaR<- 2.33*sqrt(t(W_tm) %*% cov_matrix %*% W_tm)*100
VaR
```
